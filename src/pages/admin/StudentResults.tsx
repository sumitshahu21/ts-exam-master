...
import type { Result } from '../../services/adminService';
...
function StudentResults() {
  ...
  const loadInitialData = async () => {
    try {
      ...
      if (examId) {
        ...
        await fetchResults({ examId: parseInt(examId) });
      } else {
  ...
  const handleExportCSV = async () => {
    try {
      toast.success('CSV export functionality will be implemented soon.');
    } catch (error: any) {
  ...
  const stats = {
    ...
    passedAttempts: results.filter(r => r.is_passed || ((r.total_score || r.percentage || 0) >= (r.passing_score || 70))).length,
    averageScore: results.filter(r => r.total_score !== null || r.percentage !== null).reduce((sum, r) => sum + (r.total_score || r.percentage || 0), 0) / results.filter(r => r.total_score !== null || r.percentage !== null).length || 0
  };
