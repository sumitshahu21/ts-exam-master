  ...
  try {
    switch (questionType) {
      ...
      default:
        ...
        formattedAnswer = {
          selected_options: studentAnswers.map(opt => `opt${opt}`),
          correct_options: correctAnswers.map(opt => `opt${opt}`),
          is_correct: isCorrect,
          question_marks: totalMarks,
          marks_earned: pointsEarned,
          time_taken_to_answer: 0
        };
        break;
      ...
      case 'drag-drop':
        ...
        if (questionData.type === 'matching') {
          ...
          if (studentAnswer && typeof studentAnswer === 'object') {
            Object.entries(studentAnswer).forEach(([dragItem, dropTarget]) => {
              studentPairs.push({ drag_item: dragItem, drop_target: dropTarget });
            });
          }
          ...
          correctPairs.forEach(correctPair => {
            const studentPair = studentPairs.find(sp => 
              sp.drag_item === correctPair.drag_item && sp.drop_target === correctPair.drop_target
            );
            if (studentPair) correctCount++;
          });

          isCorrect = correctCount === correctPairs.length && studentPairs.length === correctPairs.length;
          pointsEarned = isCorrect ? totalMarks : (correctCount / correctPairs.length) * totalMarks;
          ...
          formattedAnswer = {
            ...
            is_correct: isCorrect,
            question_marks: totalMarks,
            marks_earned: Math.round(pointsEarned * 100) / 100,
            time_taken_to_answer: 0
          };
        } else if (questionData.type === 'ordering') {
          // Ordering format
          ...
          // Default drag-drop format
          const correctMappings = questionData.correctMappings || {};
          const studentMappings = studentAnswer || {};
          ...
          isCorrect = correctCount === totalMappings;
          pointsEarned = isCorrect ? totalMarks : (correctCount / totalMappings) * totalMarks;

          const studentPairs = Object.entries(studentMappings).map(([drag_item, drop_target]) => ({
            drag_item, drop_target
          }));
          const correctPairs = Object.entries(correctMappings).map(([drag_item, drop_target]) => ({
            drag_item, drop_target
          }));
      ...
      case 'case-study':
        // Handle case study with multiple sub-questions
        const subQuestions = questionData.subQuestions || [];
        const studentSubAnswers = studentAnswer.responses || studentAnswer.subAnswers || {};
        ...
        subQuestions.forEach((subQ, index) => {
          ...
          if (subQ.type === 'single-choice' || subQ.question_type === 'single-choice') {
            ...
            subIsCorrect = subAnswer === correctAnswer;
            subMarksEarned = subIsCorrect ? subMarks : 0;
          } else if (subQ.type === 'multiple-choice' || subQ.question_type === 'multiple-choice') {
            const correctAnswers = subQ.correctAnswers || subQ.correct_answers || [];
            const studentAnswers = Array.isArray(subAnswer) ? subAnswer : [subAnswer];
            ...
            // Text-based sub-question - needs manual review
            subIsCorrect = false;
            subMarksEarned = 0;
          }
          ...
          responses.push({
            sub_question_id: subId,
            student_answer: subAnswer || "",
            correct_answer: subQ.correctAnswer || subQ.correct_answer || "Manual review required",
            is_correct: subIsCorrect,
            question_marks: subMarks,
            marks_earned: subMarksEarned
          });
        ...
        formattedAnswer = {
          case_study_id: questionData.caseStudyId || `cs_${Date.now()}`,
          responses: responses,
          total_marks: totalSubMarks,
          marks_earned: earnedSubMarks,
          time_taken_to_answer: 0
      ...
      case 'short-answer':
        // For text answers, we need manual review
        const correctAnswer = questionData.correctAnswer || questionData.sample_answer || "";
        const studentText = typeof studentAnswer === 'string' ? studentAnswer : String(studentAnswer || '');
